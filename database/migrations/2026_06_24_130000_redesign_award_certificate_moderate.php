        ...
        if (!empty($awardTemplateIds)) {
            ...
            foreach ($generates as $generate) {
                ...
                if ($appreciation) {
                    ...
                    $companyName = e($companyObj ? $companyObj->name : '');

                    $logoUrl = e($companyObj ? $companyObj->light_logo_url : '');
                    $logoHtml = $logoUrl ? '<img src="' . $logoUrl . '" style="max-height: 30px; max-width: 130px; display: block; margin: 0 auto; object-fit: contain;" />' : '';

                    $priceAmount = e($appreciation->price_amount ? \App\Classes\Common::formatAmountCurrency($companyObj->currency, $appreciation->price_amount) : '');
                    $profileImageUrl = '';
